...
test('claim controller fails closed and rechecks wallet and allowance before sending',async()=>{
 ...
 let fields,account,chain,nftOwner,claimed,sendCount,sent,receiptStatus,writeError,nodes,events;
 const setup=async({pending=false}={})=>{
  nodes={};events={};account=owner;chain='0x1237';nftOwner=owner;claimed=0n;sendCount=0;receiptStatus='success';writeError=null;
 ...
 try{
  ...
  await nodes['#claim-send'].onclick();assert.equal(sendCount,1);assert.equal(sent.value,2n*CLAIM_RULES.fee);assert.deepEqual(sent.args[1],[1n,1n]);assert.match(nodes['#claim-message'].textContent,/Confirmed: 200/);assert.equal(nodes['#claim-send'].disabled,true);
