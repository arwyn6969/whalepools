...
const worker=(await import(localAuth?'../build/worker.mjs':'../build/launch-worker.mjs')).default;
await mkdir(path.join(app,'work'),{recursive:true});
const DB=database(path.join(app,'work',localAuth?'club.sqlite':'arcade.sqlite'));
