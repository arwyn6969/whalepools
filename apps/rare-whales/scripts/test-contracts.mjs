...
import {createPublicClient,createWalletClient,http,parseEther,getContractAddress,zeroAddress} from 'viem';
...
const reject=(address,name,args,account,value=0n,type='WhaleWaxClaims')=>assert.rejects(()=>client.simulateContract({address,abi:C[type].abi,functionName:name,args,account,value}));
...
try{
 ...
 await check('new owner claims at next period; previous owner cannot',async()=>{assert.equal(await read(d,'currentPeriod'),1n);await reject(d,'claim',[[rare],[1n]],alice,fee);await write(d,'claim',[[rare],[1n]],bob,fee);assert.equal(await read(d,'claimedPeriods',[rare,1n]),3n);});
 await at(start+11n*30n*86400n);
 await check('skipped periods do not accumulate',async()=>{const before=await read(token,'balanceOf',[alice],'WhaleWax');await write(d,'claim',[[rare],[2n]],alice,fee);assert.equal(await read(token,'balanceOf',[alice],'WhaleWax'),before+reward);});
