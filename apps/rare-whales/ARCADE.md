...
The browser inventory reader uses the authenticated session wallet only. These collections do not support ERC-721 owner enumeration. The browser reads incoming/outgoing Transfer logs from the official Robinhood RPC, orders the most recent event per token, and requires the resulting count to equal each on-chain balance at the same block (head minus two). Balance and history reads use the official public endpoint; both chain IDs must be 4663. Hosted inventory reads failed their live check and were removed. Publish-time ownership checks still use the server’s configured dRPC endpoint. This avoids a paid indexer. The public log endpoint is an availability dependency: errors, incomplete history or more than 20,000 combined transfer records per collection show a retry state, never a fabricated empty inventory. Publishing still independently checks every `ownerOf`.
